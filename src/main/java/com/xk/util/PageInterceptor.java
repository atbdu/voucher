...
public class PageInterceptor implements Interceptor {
    ...
    private String getPageSql(Page page, String sql) {
        ...
        if ("mysql".equalsIgnoreCase(databaseType)) {
            ...
            String sqls=getOraclePageSql(page, sqlBuffer);
            System.out.println("pageSql: "+sqls);
            return sqls;
        }
    ...
    private String getOraclePageSql(Page page, StringBuffer sqlBuffer) {
        ...
        if(page.getOrderField()!=null&&!page.getOrderField().equals("")&&sqlBuffer.indexOf("order by")==-1){//排序
            orderField=page.getOrderField();       //为了兼容原来页面类似表别名的ts.status的排序
        ...
        //上面的Sql语句拼接之后大概是这个样子：
        //select * from (select u.*, rownum r from (select * from (select * from t_user) o order by 1 desc) u where rownum < 31) where r >= 16
        return sqlBuffer.toString();
    }
    ...
    private void setTotalRecord(Page page,MappedStatement mappedStatement, Connection connection) {
        ...
        String countSql = this.getCountSql(sql,page);
        //通过BoundSql获取对应的参数映射
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if(page.getOtherCtSql()!=null&&!page.getOtherCtSql().equals("")){
                    String otherValue=rs.getString(2);
                    page.setOtherValue(otherValue);
                }
                //给当前的参数page对象设置总记录数
    ...
    private String getCountSql(String sql,Page page) {
        String sqls="select count(*) from(" +sql+") as countTable";
        if(page.getOtherCtSql()!=null&&!page.getOtherCtSql().equals("")){
            sqls="select count(*),"+page.getOtherCtSql()+" from(" +sql+") as countTable";
        }
        return sqls;
    }
