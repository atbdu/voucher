...
*            key数组
     ...
     * @return
     */
    public static boolean export(HttpServletResponse response, List<Map<String, Object>> list, String[] lables, String[] fields, String title) {
...
//            setSheet(sheet);
     ...
     * @param headTitle
     *            文件标题
     * @param payChannel_list
     *            文件标题
     */
    public static boolean titleExport(HttpServletResponse response,List<Map<String, Object>> list,String[] lables,String[] fields,String title,String headTitle,List<Map<String,Object>> payChannel_list) {
        response.setContentType("application/octet-stream");//告诉浏览器输出内容为流
        String filename = "";
        try {
            filename = new String(title.getBytes("UTF-8"), "ISO_8859_1");
        ...
        try {
            ...
            for (int i = 0, j = 0; i < payChannel_list.size(); i++) {

                Sheet sheet = sxssfWorkbook.createSheet(payChannel_list.get(i).get("channel_short_name") + "对账表");
                // 创建第一行,作为标题
                ...
                // 遍历创建行,导出数据
                for (int rownum = 1, num = 1; rownum <= list.size(); rownum++) {
                    if (list.get(rownum - 1).get("pay_channel").equals(payChannel_list.get(j).get("channel_short_name"))) {
...
//        xssfCellStyle.setFont(font);
        ...
        xssfCellStyle.setFillForegroundColor(IndexedColors.TEAL.getIndex());
